...
const DB_URL = "https://reclamos-db-default-rtdb.firebaseio.com/reclamos";
...
export default function Reclamos() {
  ...
  const [form, setForm] = useState({ orden: "", cliente: "", etiqueta: false, productos: "", productosFaltantes: "", notas: "", estado: "pendiente" });
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    if (typeof window !== "undefined") {
      if (!localStorage.getItem("loggedIn")) { router.push("/"); return; }
  ...
  async function handleSubmit() {
    if (!form.orden || !form.productos) return alert("Completá el número de orden y los productos.");
    setSaving(true);
    const nuevo = { ...form, responsable: username, fechaCreacion: new Date().toISOString() };
    await fetch(`${DB_URL}.json`, { method: "POST", body: JSON.stringify(nuevo) });
    setSaved(true);
    setSaving(false);
    setTimeout(async () => {
      setSaved(false);
  ...
  async function updateEstado(fireId, estado) {
    const r = reclamos.find(r => r.fireId === fireId);
    await fetch(`${DB_URL}/${fireId}.json`, { method: "PATCH", body: JSON.stringify({ estado, fechaActualizacion: new Date().toISOString() }) });
    const updated = reclamos.map(r => r.fireId === fireId ? { ...r, estado } : r);
    setReclamos(updated);
    setSelected({ ...r, estado });
  }

  async function deleteReclamo(fireId) {
    if (!confirm("¿Eliminar este reclamo?")) return;
    await fetch(`${DB_URL}/${fireId}.json`, { method: "DELETE" });
  ...
  const counts = { pendiente: reclamos.filter(r => r.estado === "pendiente").length, "en proceso": reclamos.filter(r => r.estado === "en proceso").length, resuelto: reclamos.filter(r => r.estado === "resuelto").length };
    ...
    <div style={{ minHeight: "100vh", background: "#0f1117", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: "#e2e8f0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .field input, .field textarea, .field select { width: 100%; background: #1a1d27; border: 1.5px solid #2d3748; border-radius: 8px; padding: 10px 14px; color: #e2e8f0; font-size: 14px; transition: border 0.15s; outline: none; }
        .field input:focus, .field textarea:focus, .field select:focus { border-color: #3b82f6; }
        .field textarea { resize: vertical; min-height: 80px; }
        .card { background: #1a1d27; border: 1px solid #2d3748; border-radius: 12px; padding: 18px; cursor: pointer; transition: all 0.18s; }
        .card:hover { border-color: #3b82f6; background: #1e2235; transform: translateY(-2px); }
        .badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase; }
        .toggle { width: 44px; height: 24px; background: #2d3748; border-radius: 12px; cursor: pointer; position: relative; transition: background 0.2s; border: none; }
      ...
      <div style={{ background: "#13161f", borderBottom: "1px solid #2d3748", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button className="btn btn-ghost" onClick={() => setView("lista")}>Lista</button>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {view === "lista" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
            {[["Pendientes", "pendiente", "#f59e0b"], ["En Proceso", "en proceso", "#3b82f6"], ["Resueltos", "resuelto", "#10b981"]].map(([label, key, color]) => (
              <div key={key} onClick={() => setFilter(filter === key ? "todos" : key)} style={{ background: "#1a1d27", border: `1px solid ${filter === key ? color : "#2d3748"}`, borderRadius: 12, padding: "16px 20px", textAlign: "center", cursor: "pointer" }}>
        ...
        {view === "nuevo" && (
          <div style={{ background: "#1a1d27", border: "1px solid #2d3748", borderRadius: 16, padding: 28 }}>
            ...
            {saved ? (
              <div style={{ background: "#10b981", color: "white", borderRadius: 10, padding: "14px 20px", textAlign: "center", fontWeight: 600 }}>✓ Reclamo guardado correctamente</div>
            ) : (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  ...
                  <div className="field"><label>Estado</label><select value={form.estado} onChange={e => setForm({ ...form, estado: e.target.value })}><option value="pendiente">Pendiente</option><option value="en proceso">En Proceso</option><option value="resuelto">Resuelto</option></select></div>
                </div>
            ...
            <div style={{ borderTop: "1px solid #2d3748", paddingTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {["pendiente", "en proceso", "resuelto"].map(s => (
                  <button key={s} onClick={() => updateEstado(selected.fireId, s)} style={{ background: selected.estado === s ? STATUS_COLORS[s] : "transparent", color: selected.estado === s ? "white" : STATUS_COLORS[s], border: `1.5px solid ${STATUS_COLORS[s]}`, padding: "6px 14px", fontSize: 12, borderRadius: 8, cursor: "pointer", fontWeight: 600, fontFamily: "inherit" }}>{s}</button>
                ))}
              </div>
              <button className="btn btn-danger" onClick={() => deleteReclamo(selected.fireId)}>🗑 Eliminar</button>
            </div>
          </div>
        )}
      </div>
